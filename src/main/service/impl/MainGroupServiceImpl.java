...
public class MainGroupServiceImpl implements MainGroupService {
    ...
    public ArrayList<AllGroupDetail> getAllGroupDetails() throws SQLException {
        Statement stm = null;
        try {
            String sql = "SELECT * FROM academicyearandsemester ayas ,maingroup mg ,\n" +
                    "subgroup sg , programme p ,notavailablegroup nag WHERE \n" +
                    "ayas.id = mg.semid AND p.programmeid = mg.programmeid GROUP BY \n" +
                    "sg.subgroupid;";
            stm = connection.createStatement();
            try (ResultSet rst = stm.executeQuery(sql)) {
